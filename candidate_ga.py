...
import os
from utils import RangedJSONEncoder

class Candidate_GA:

    def __init__(self, candidate_id, start_time_str, runtime_spec, network_spec=None):
        self._candidate_id = candidate_id
        self._base_logdir = os.path.join(runtime_spec['logdir'], str(start_time_str))
    ...
    def _crossover_uniform(self, crossover_rate, other_candidate, uniform_method):
        if random.uniform(0, 1) <= crossover_rate:
            self.network_spec['crossover']['rate'] = other_candidate.network_spec['crossover']['rate']

        if random.uniform(0, 1) <= crossover_rate:
            self.network_spec['mutation']['rate'] = other_candidate.network_spec['mutation']['rate']

        if random.uniform(0, 1) <= crossover_rate:
            self.network_spec['selection_strategy']['best_win_rate'] = \
            other_candidate.network_spec['selection_strategy']['best_win_rate']

        if random.uniform(0, 1) <= crossover_rate:
            self.network_spec['selection_strategy']['tournament_size'] = \
            other_candidate.network_spec['selection_strategy']['tournament_size']
    ...
    def get_fitness(self):
        if self._fitness is None:
            ...
            for i in range(self.network_spec['number_of_generation']):
                ...
                gen.selection()
                if self._best_candidate is None or gen.best_candidate_forever.get_fitness() >= self._best_candidate.get_fitness():
                ...
                self._fitness = 0.6*gen.best_candidate.get_fitness() + 0.4*gen.fitness_avg

        file_loc = os.path.join(self.network_spec['RUNTIME_SPEC']['logdir'], "ga.json")
        with open(file_loc, 'w') as fp:
            fp.write(str(self._fitness)+"\n")
            fp.write(RangedJSONEncoder().encode(self.network_spec))
    ...
    def to_next_generation(self, generation):
        ...
        self.network_spec['RUNTIME_SPEC']['logdir'] = os.path.join(self._base_logdir, generation_dir, id_dir)
        #self.network_spec.update(self.network_spec)
